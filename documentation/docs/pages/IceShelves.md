...
#### Grid
The grid was generated using the `ocean_grid_generator` (see https://github.com/claireyung/mom6-panAn-iceshelf-tools/issues/7)

#### Topography
Bottom topography is generated from [Charrassin et al. (2025)](https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/generate-obcs/ACCESS-OM2_panan_boundary_forcing_8km.ipynb) product, stitched to GEBCO 2024 where the Charrassin product is not available. The topography generation process required regridding from the Antarctic polar grid EPSG:3031 to latitude-longitude coordinates and is described in https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/generate-draft/Generate-Charrassin-bathy.ipynb and https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/generate-draft/process-topo.ipynb. Unfortunately, the normal OM3 topography generation workflow cannot be applied here so we manually set maximum and minimum depths, perform deseas to get rid of inland lakes, and create masks. These notebooks are also used for ice shelf configuration in development. Once these new topography and masks are generated, the nuopc mesh files also need to be generated with `om3-scripts`, as well as the bottom roughness and tidal amplitude. 

#### Initial Conditions and Boundary Conditions
Initial conditions were generated using a restart at the start of year 2 of an ACCESS-OM2-01 RYF90-91 configuration spun up from WOA13, as used in the COSIMA MOM6-SIS2 pan-Antarctic models (https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/initial-conditions/ACCESSOM2_IC_into_8km_grid.ipynb). Boundary conditions use daily data from the second year of this run, (https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/generate-obcs/ACCESS-OM2_panan_boundary_forcing_8km.ipynb). Salt restoring is regridded from ACCESS-OM2-01, as in the COSIMA MOM6-SIS2 models (https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/initial-conditions/salt-restoring.ipynb).

#### Model parameters

MOM6 parameters are a combination of ACCESS-OM3 25km choices, MOM6-SIS2 COSIMA pan-Antarctic models and regional models from GFDL. We use a Wright equation of state (https://github.com/claireyung/mom6-panAn-iceshelf-tools/issues/13) and higher resolution density coordinates (taken from MOM6-SIS2 COSIMA pan-Antarctic model). These parameter choices are found in https://github.com/claireyung/access-om3-configs/blob/8km_jra_ryf_obc2-sapphirerapid-Charrassin-newparams-rerun-Wright-spinup-accessom2IC-yr9/MOM_override_newparams .
...
### Timestepping
In this configuration, an ocean baroclinic timestep of 600s is used, with a relatively short thermodynamic timestep of 1200s. This is due to problems with the MOM6-SIS2 COSIMA pan-Antarctic broundary conditions if the tracer timestep is too long (https://github.com/COSIMA/mom6-panan/issues/28). The behaviour of the boundary condition under different thermodynamic timesteps has not been explored in this configuration. 
