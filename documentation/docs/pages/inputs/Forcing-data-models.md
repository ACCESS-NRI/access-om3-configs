...
JRA55-do runoff provides daily mean liquid and frozen runoff fields, although the runoff at many locations in the dataset is updated less frequently than daily. In the source data, all frozen runoff is distributed at the ocean surface of the Antarctic/Greenland coastlines without spreading (see issue [#404](https://github.com/ACCESS-NRI/access-om3-configs/issues/404)). 

Similar to atmospheric forcing, runoff data is first interpolated in time, second remapped from DROF to the mediator, and lastly from the mediator to the ocean (MOM6). The first and second mapping step is similar to the atmospheric forcing, namely using linear interpolation in time and conservative interpolation spatially. The last stage remapping moves from the om3 grid without a landmask to the same grid with a landmask. For liquid runoff and frozen runoff, the last stage is approached differently for each:

For *liquid runoff*, whilst the volume of the JRA55do forcing is unaffected, two changes are made to where the runoff enters the ocean:

1. Near a small number of major river mouths (Amazon, Tigre, Kobuk/Noatak, Yenisey, Lena, Khatanga — see `SPREAD_POINTS` in the [script](https://github.com/ACCESS-NRI/om3-scripts/blob/main/mesh_generation/generate_rof_weights.py)), where a large volume of runoff would otherwise be dumped into a single grid cell, it is distributed across several nearby ocean cells with exponentially-decaying weight by distance from the source. This avoids instabilities in the ocean model which can occur to unrealistically low salinity, or large variations in sea surface height.
1. Differences in landmask between the incoming data on the JRA55-do grid and the OM3 grids can cause runoff to be remapped to land cells or on cells away from the coastlines. When runoff would be placed on land cells or non-coastal ocean cells and is not being spread, the volume of runoff is moved to a nearby coastal ocean cell using pre-generated weights from the [generate_rof_weights.py](https://github.com/ACCESS-NRI/om3-scripts/blob/main/mesh_generation/generate_rof_weights.py) script. _generate_rof_weights.py_ selects the nearest coastal ocean cell by a BallTree algorithm using Haversine distances (i.e. the shortest distance on a sphere). This ensures that the full global volume of runoff enters the ocean, and only at cells which abut land cells.
...
Note that the MOM6 parameter [DO_RIVERMIX](https://github.com/search?q=repo%3AACCESS-NRI%2Faccess-om3-configs+DO_RIVERMIX&type=code) is True in some configurations. This adds additional turbulent kinetic energy to increase vertical mixing of the runoff forcing.
