...
```
python ocean_grid_generator.py -r 12 --no_south_cap --ensure_nj_even --bipolar_lower_lat 65 --mercator_lower_lat -75 --mercator_upper_lat 65 --match_dy so --shift_equator_to_u_point --south_ocean_lower_lat -86.5 --lower-lon -280.0 -f ocean_hgrid.nc
```
...
We generate initial conditions by regridding Yamazaki et al. 2025 and ACCESS-OM2-01 data from the Jan 1 after second year restart (similar to WOA), see [this notebook](https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/initial-conditions/Yamazaki_IC_into_8km_grid-Copy1.ipynb). Alternatives could be WOA data, but one must be careful about which temperature/salinity variable you use, and we've found the WOA data is [quite cold on the Amundsen shelf](https://github.com/claireyung/mom6-panAn-iceshelf-tools/issues/45). While extrapolating southwards, we also manually extrapolate in a different direction for parts of some cavities (Weddell, Ross, Larsen C) where otherwise the southward-extrapolated initial conditions are very warm in the cavities.
...
Salt restoring [used a similar method](https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/initial-conditions/salt-restoring.ipynb) and data based on WOA13. We use a [salt restoring mask](https://github.com/claireyung/mom6-panAn-iceshelf-tools/blob/main/generate-draft/process-topo-8k-minimal-topoedited.ipynb) defined by 1000m isobath (the continental shelf), saved as `salt_restore_mask.nc`.
