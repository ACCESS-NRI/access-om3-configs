...
### Grid description
The 25km configuration uses a tripolar grid to avoid a singularity at the North Pole (`TRIPOLAR_N = True`). The domain is zonally periodic, open at the north via the tripolar fold and closed in the south (`REENTRANT_X = True`, `REENTRANT_Y = False`). The horizontal grid has 1440x1152 tracer points. This is closely aligned with prior models, such as ACCESS-OM2-025 and GFDL OM4/OM5 (1440x1080) and provides eddy-permitting detail in the ocean while maintaining numerical stability. See [Grids](/inputs/Grids/) for more information. 
