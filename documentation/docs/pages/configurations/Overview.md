...
#### Model exectuable version
  - [`modules: load:`](https://github.com/search?q=repo%3AACCESS-NRI%2Faccess-om3-configs+path%3Adoc%2Fconfig.yaml+load:&type=code) in `config.yaml`. This sets the [released version](https://github.com/ACCESS-NRI/ACCESS-OM3/releases) if using a pre-build executable. If using a prerelease [build](../../infrastructure/Building/#ad-hoc-builds) or local builds, set both `modules: use` and `modules:load` to match.
#### Coupling
