...
#### Model executable
  - [`exe`](https://github.com/search?q=repo%3AACCESS-NRI%2Faccess-om3-configs+path%3Adoc%2Fconfig.yaml+exe:&type=code) in `config.yaml`. Pre-built executables are available in `/g/data/ik11/inputs/access-om3/bin/` or you can [build your own](pages/Building). Executable names indicate the available model components and the [git hash of the source code](https://github.com/COSIMA/access-om3/commits/) used. Avoid using the `Debug` versions for production runs as they are much slower.
#### Coupling
...
- **components and fields to couple**
  - See the coupling architecture [here](pages/Architecture)
...
#### timesteps
  - there is a complex set of interrelated timesteps - see [here](pages/NUOPC-driver#time-steps) and [here](https://earthsystemmodeling.org/docs/release/ESMF_8_3_1/NUOPC_refdoc/node3.html#SECTION00035000000000000000) to understand how they interact
  - coupling and driver timesteps - see [here](pages/NUOPC-driver#coupling-and-driver-time-step)
  ...
  - MOM6 timestepping - see [here](pages/NUOPC-driver#mom6-time-steps)
  ...
  - CICE6 timestepping - see [here](pages/NUOPC-driver#cice6-time-steps)
    ...
    - The thermodynamic timestep is determined by the coupling (and driver) timestep (so _`dt` should **not** be explicitly set in `ice_in`_ - see [here](pages/NUOPC-driver#cice6-time-steps))
...
#### forcing data
  - see the [Forcing](pages/Forcing-data-models) page
  - atmospheric forcing
