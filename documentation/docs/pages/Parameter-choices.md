...
We ensure salinity never goes negative by setting `BOUND_SALINITY = True`. In coupled models, sea-ice formation and melting can generate large salinity fluxes at the ocean surface. This setting clips salinity at a minimum of `MIN_SALINITY = 0.0`. However, if the lower bound is hit, it clips the salinity value and discards any excess, potentially violating salt conservation in these rare edge cases, which can cause sea-ice model crashes first with thermodynamic conservations errors. We also set SALINITY_UNDERFLOW = 0.0, which resets very small salinity values to exactly zero.
