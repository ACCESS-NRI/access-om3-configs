...
There are two sorts of debug builds also possible

- building with `build_type=Debug` can be useful for getting more information on model crashes as it adds extra runtime checks, for example to find uninitialized variables. These should not be used for general model runs as they are much slower. These debug builds are largely untested, but will probably get fixed up over time.
- building for an interactive debugger. We suggest Linaro DDT and the Linaro Forge remote client. [Instructions](https://docs.access-hive.org.au/models/build_a_model/build_source_code/#debugging-a-model) are available on the ACCESS-Hive docs.
